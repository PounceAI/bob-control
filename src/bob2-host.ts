...
// The production Bob2Host — the binding from the in-process driver's host seam to the real VS Code / Bob
// APIs. Kept out of bob2-driver.ts (which stays vscode-free and unit-testable) and built over an injected
// `vscode` surface, so it's testable with a fake and the extension passes the real module.
 ...
 * extension is present (its activate() ran) — null otherwise, the negative signal isBob2Window uses to
 * fall back to the 1.x pipe driver. `workspaceFolder()` is the first open folder's fsPath (blank → null).
 * Validating the exports shape is left to the driver (isBob2Window/connect).
 */
